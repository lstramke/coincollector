<script lang="ts">
    ...
    const { group, selectedId, onSelect, onGroupToggle } = $props<{
        group: Group;
        selectedId?: string;
    ...
    function toggleOpen() {
        if (open && selectedId && group.collections.some((c: { id: string; }) => c.id === selectedId)) {
            onGroupToggle();
        }
        open = !open;
    }
...
<section class="text-sm text-[var(--text-white)]">
    <button
        type="button"
        class="flex w-full items-center justify-between rounded-lg px-3 py-2 text-left font-semibold text-[var(--text-white)]/90 transition-colors hover:bg-[var(--hover-state)]/40"
            ...
            {#each group.collections as collection (collection.id)}
                <li>
                    <button
                        ...
                        onpointerup={() => onSelect(collection.id)}
                    >

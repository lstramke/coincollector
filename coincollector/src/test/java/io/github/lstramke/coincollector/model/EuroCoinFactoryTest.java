...
public class EuroCoinFactoryTest {
	...
	private static Stream<FromDbEntryTestcase> fromDbEntryTestcases(){
		return Stream.of(
			new FromDbEntryTestcase("coin-1", 2002, 100, "DE", "A", "1 Euro Münze", "col-1", false, null,
				"valid: proper row creates coin with preserved id"),
			new FromDbEntryTestcase(null, 2002, 100, "DE", "A", "desc", "col-1", false, null,
				"valid: null id -> autogenerated correct id"),
			new FromDbEntryTestcase("   ", 2002, 100, "DE", "A", "desc", "col-1", false, null,
				"valid: empty id -> autogenerated correct id"),
			new FromDbEntryTestcase("coin-2", 1998, 100, "DE", "A", "desc", "col-1", false, SQLException.class,
				"invalid: year too small -> wrapped to SQLException with ISE cause"),
			new FromDbEntryTestcase("coin-3", 2002, 999, "DE", "A", "desc", "col-1", false, SQLException.class,
				"invalid: unknown coin value -> wrapped to SQLException with IAE cause"),
			new FromDbEntryTestcase("coin-4", 2002, 100, "ZZ", "A", "desc", "col-1", false, SQLException.class,
				"invalid: unknown country iso -> wrapped to SQLException with IAE cause"),
			new FromDbEntryTestcase("coin-5", 2002, 100, "DE", "Z", "desc", "col-1", false, SQLException.class,
				"invalid: unknown mint mark -> wrapped to SQLException with IAE cause"),
			new FromDbEntryTestcase("coin-6", 2002, 100, "DE", "A", "desc", null, false, SQLException.class,
				"invalid: null collection id -> wrapped to SQLException with ISE cause"),
			new FromDbEntryTestcase("ignored", 2002, 100, "DE", "A", "desc", "col-1", true, SQLException.class,
	...
	void testFromDataBaseEntry(FromDbEntryTestcase testcase) throws Exception {
		...
		if (testcase.expectedException != null){
			...
			assertEquals(testcase.mintMark, coin.getMint().getMintMark());
